...
public class MultiImagePreviewActivity extends FragmentActivity implements MediaItemsDataSource.MediaItemProvider {
    ...
    private ArrayList<ImageItem> mIgnoreSelectList;
    ...
    private boolean isIntentDataFailed() {
        ...
        if (selectConfig.isCanEditPic){
            mIgnoreSelectList = new ArrayList<>();
            for (ImageItem imageItem:mSelectList){
                mIgnoreSelectList.add(imageItem);
            }
        }
...
//        }
    ...
    /**
     * //如果是脱敏处理 在预览时取消的选中,退出预览时候删除脱敏照片
     */
    private void deleteEidtPic() {
        if (selectConfig.isCanEditPic) {
            if (mSelectList.size()==0){
                ...
                mIgnoreSelectList.removeAll(mSelectList);
        ...
        if (mIgnoreSelectList!=null){
            mIgnoreSelectList.clear();
        }
